...
/*
	...
	通过切片生成的切片会指向同一个地址,如果改变其中一个会改变另一个
	但是当某个子切片扩容后就会整体的copy到新的地址,就与之前的切片无关
	详细:https://golang.design/go-questions/slice/vs-array/
*/
...
func main() {
	...
	*/
	...
	/*
		slice的扩容
		1.18之前:[0, 1024]内翻倍扩容,大于1024则每次扩容1/4
		1.18之后:超过256就变成 1/4 * cap +192,256以内则是翻倍
	*/
	sss := make([]int, 0)
	old := 0
	for len(sss) < 2048 {
